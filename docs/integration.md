...
النتيجة الناجحة تحتوي `choices[0].message.content` و`usage`. قد يحتوي الرد على `images: []` في طلبات النص، لكن الخادم لا يفحص DOM الخاص بالصور أثناء طلبات النص.

> **حدود المسارات:** استخراج الصور من DOM مفعّل فقط عندما يكون الطلب صورة. طلبات النص والبحث تعتمد على نص المساعد ونتيجة البحث، ولا تستدعي image-count أو image locators، مما يمنع تسرب favicon أو صورة قديمة إلى هذه المسارات.
...
يستخدم المستهلك الطلب نفسه مع prompt توليد صورة. يحدد الخادم طلب الصورة من `output_type=image` أو modalities أو مؤشرات prompt مثل `generate image` و`أنشئ صورة`، ثم يفعّل استخراج DOM للصورة فقط. النتيجة الناجحة تحتوي عنصرًا واحدًا أو أكثر في `images`. لا تُعتبر favicon أو avatar أو صورة بلا marker `Generated image` صورة مولدة.
...
النص عادة أسرع من الصورة. استخدم مهلة لا تقل عن 240 ثانية لطلبات الصورة، وطبّق إعادة محاولة محدودة على أخطاء الشبكة و5xx فقط. لا تعِد المحاولة بلا نهاية ولا تعِد طلبًا بعد 401/403 قبل تدوير Secret أو إصلاح الإعداد. إذا لم توجد صورة مولدة بعد الانتظار، يجب إعادة خطأ واضح بدل إرجاع favicon أو asset قديم على أنه صورة.
